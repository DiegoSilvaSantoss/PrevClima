...
campoCidade.addEventListener('input', async (e) => {
  ...
  const res = await fetch(
    `https://api.openweathermap.org/geo/1.0/direct?q=${valor}&limit=5&appid=${chaveApi}`
  );
...
function mostrarSugestoes(cidades) {
  ...
  cidades.forEach(cidade => {
    ...
    const bandeira = `https://flagsapi.com/${cidade.country}/flat/32.png`;
    
    li.innerHTML = `
      <img src="${bandeira}" width="20" />
